...
function displayMenuItems(menuItems) {
  let display = menuItems.map(function (item) {
    return `<div class="${item.category}">
            <img src="${item.img}">
                <section>
                    <header><h4>${item.title}</h4><h4>${item.price}</h4></header>
                    <article>
                        ${item.desc}
                    </article>
                </section>
...
function displayButtons() {
  ...
  let displaybtn = categories.map((m) => {
    return `<button type="button" class="filter-btn" data-id=${m}>${m}</button>`;
  });

  displaybtn = displaybtn.join("");
  btncont.innerHTML = displaybtn;

  const filterBtns = btncont.querySelectorAll(".filter-btn");
